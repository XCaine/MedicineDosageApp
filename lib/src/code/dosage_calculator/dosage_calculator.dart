...
enum DosageCalculatorMode { bestMatchesAcrossMultipleDrugs, bestMatchForEachDrug }
...
class DosageCalculator {
  ...
  DosageCalculator({required DosageSearchWrapper searchWrapper}) : _searchWrapper = searchWrapper;
  ...
  Future<DosageResultSetWrapper> findMatchingPackageSets(DosageCalculatorMode mode) async {
    ...
    ''');
    List<Medication> feasibleMedicineInstances =
        distinctMedicines.map((medicine) => Medication.fromJson(medicine)).toList();
    ...
    for (Medication medicineInstance in feasibleMedicineInstances) {
      ...
      ''');
        ...
        //deduplication
        var seen = <int>{};
        var uniqueMatchingPackageInstances =
            feasiblePackageInstances.where((package) => seen.add(package.count!)).toList();

        int target = _searchWrapper.dosagesPerDay! *
            (_searchWrapper.searchByDates
                ? _searchWrapper.dateEnd!.difference(_searchWrapper.dateStart!).inDays
                : _searchWrapper.numberOfDays!);
        ...
        for (List<int> packagesForOption in allOfferedOptions) {
          List<Package> packagesForResultWrapper = List.from(packagesForOption.map((singlePackageCount) =>
              feasiblePackageInstances.firstWhere(
                  (instance) => instance.count! == singlePackageCount && instance.medicineId == medicineInstance.id)));
        ...
        _logger.severe(
            'Failed to get proposed package variants for medicine ${medicineInstance.id}: ${medicineInstance.productName}',
            e,
            stackTrace);
      }
    }
    ...
    if (mode == DosageCalculatorMode.bestMatchForEachDrug) {
      ...
      wrapper = DosageResultSetWrapper(resultSets: results);
    } else if (mode == DosageCalculatorMode.bestMatchesAcrossMultipleDrugs) {
      wrapper = DosageResultSetWrapper(resultSets: results.take(_maxNumberOfResults).toList());
    } else {
